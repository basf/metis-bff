...
async function getUserDataSources(req, res, next) {
    ...
    try {
        req.session.datasources = await selectDataSourcesByUserIdAndRole(req.user.id, req.user.roleSlug);
    } catch(error) {
        return next({ error });
    }
...
async function getUserCalculations(req, res, next) {
    ...
    try {
        req.session.calculations = await selectCalculationsByUserIdAndRole(req.user.id, req.user.roleSlug);
    } catch(error) {
        return next({ error });
    }
...
async function getUserCollections(req, res, next) {
    ...
    if (!req.session.collections) {
        return next({ status: StatusCodes.UNPROCESSABLE_ENTITY, error: 'Collections are not available' });
    }
