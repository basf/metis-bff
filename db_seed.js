    ...
    db.schema.hasTable(COLLECTONS_TYPES_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(COLLECTONS_TYPES_TABLE, (table) => {
                ...
                table.enu('color', COLORS_ENUM, { useNative: true, enumName: 'collection_colors' });
    ...
    db.schema.hasTable(USERS_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(USERS_TABLE, (table) => {
                ...
                table.foreign('roleId', 'fk_roleId').references('id').inTable(USER_ROLES_TABLE).onDelete('CASCADE');
            });
        } else {
    ...
    db.schema.hasTable(USERS_EMAILS_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(USERS_EMAILS_TABLE, (table) => {
                ...
                table.foreign('userId', 'fk_userId').references('id').inTable(USERS_TABLE).onDelete('CASCADE');     
            });
        } else {
    ...
    db.schema.hasTable(USER_OAUTHS_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(USER_OAUTHS_TABLE, (table) => {
                table.integer('userId', FOREIGN_KEY_LENGTH).unsigned().index();
                table.enu('provider', OAUTH_PROVIDERS_ENUM, { useNative: true, enumName: 'oauth_providers' })
                table.string('providerId').unique(); 
                table.jsonb('profile').nullable();
                table.timestamps(false, true, true);

                table.primary(['userId', 'provider'], { constraintName: 'pk_user_provider' });  
                table.foreign('userId', 'fk_userId').references('id').inTable(USERS_TABLE).onDelete('CASCADE');            
            });
        } else {
    ...
    db.schema.hasTable(USER_COLLECTONS_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(USER_COLLECTONS_TABLE, (table) => {
                ...
                table.enu('visibility', VISIBILITY_ENUM, { useNative: true, enumName: 'collection_visibility' }).defaultTo(VISIBILITY_ENUM[0]);
                table.timestamps(false, true, true);

                table.foreign('userId', 'fk_userId').references('id').inTable(USERS_TABLE).onDelete('CASCADE');
                table.foreign('typeId', 'fk_typeId').references('id').inTable(COLLECTONS_TYPES_TABLE).onDelete('CASCADE');
            });
        } else {
    ...
    db.schema.hasTable(USER_SHARED_COLLECTONS_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(USER_SHARED_COLLECTONS_TABLE, (table) => {
                ...
                table.primary(['userId', 'collectionId'], { constraintName: 'pk_user_collection' });  
                table.foreign('userId', 'fk_userId').references('id').inTable(USERS_TABLE).onDelete('CASCADE');
                table.foreign('collectionId', 'fk_collectionId').references('id').inTable(USER_COLLECTONS_TABLE).onDelete('CASCADE');
            });
        } else {
    ...
    db.schema.hasTable(USER_COLLECTONS_DATASOURCES_TABLE).then((exists) => {
        if (!exists) {
            return db.schema.createTable(USER_COLLECTONS_DATASOURCES_TABLE, (table) => {
                ...
                table.primary(['collectionId', 'dataSourceId'], { constraintName: 'pk_collection_dataSource' });  
                table.foreign('collectionId', 'fk_collectionId').references('id').inTable(USER_COLLECTONS_TABLE).onDelete('CASCADE');
                table.foreign('dataSourceId', 'fk_dataSourceId').references('id').inTable(USER_DATASOURCES_TABLE).onDelete('CASCADE');
            });
        } else {
